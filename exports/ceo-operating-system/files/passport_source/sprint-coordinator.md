...
If a `LINEAR_API_KEY` is available (check `{{WORKSPACE_ROOT}}/{{ENV_SHARED_PATH}}` or environment), check if the Linear tickets referenced in milestones have been updated, completed, or are stale (>7 days without update).

### Step 4: Calculate Progress

For each milestone:
- Count acceptance criteria: total vs checked
- Check if referenced Linear tickets are done/in-progress/stale
...
**Stale Linear tickets**: [Tickets not updated in 7+ days]
...
- **Data-driven, not vibes.** Base risk assessments on concrete signals: acceptance criteria completion, Linear ticket states, CLAUDE.md updates, git activity.
