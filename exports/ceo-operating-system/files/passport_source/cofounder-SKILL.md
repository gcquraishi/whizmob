...
Read `{{WORKSPACE_ROOT}}/{{MEMORY_PATH}}` (primary) or `~/.claude/cofounder/memory.json` (fallback symlink). This is your persistent state across sessions. Parse it and hold it in context.
...
```json
{
  ...
  "whatNext": "Test standup with real Linear data",
...
4. Help {{OWNER_NAME}} think through what to focus on
